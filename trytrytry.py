...
def clone_faces(dest_image, new_face, head_mask, center_face):
    min_height = min(dest_image.shape[0], new_face.shape[0], head_mask.shape[0])
    min_width = min(dest_image.shape[1], new_face.shape[1], head_mask.shape[1])

    dest_image = dest_image[:min_height, :min_width]
    new_face = new_face[:min_height, :min_width]
    head_mask = head_mask[:min_height, :min_width]

    head_mask = head_mask.astype(float) / 255.0

    result = (1 - head_mask[:, :, np.newaxis]) * dest_image + head_mask[:, :, np.newaxis] * new_face
...
def swap_new_face(dest_image, dest_image_gray, dest_convexHull, new_face):
    ...
    small_dest_image = cv2.resize(dest_image, (width // 2, height // 2))
    small_new_face = cv2.resize(new_face, (width // 2, height // 2))
    small_head_mask = cv2.resize(head_mask, (width // 2, height // 2))

    small_result = clone_faces(small_dest_image, small_new_face, small_head_mask, center_face)

    result = cv2.resize(small_result, (width, height))
...
def set_src_image(image):
    ...
    indexes_triangle = get_triangles(convexhull=src_convexHull,landmark_points=src_landmark_points,np_points=src_np_points)
